...
async function main() {
  const markets = [
    ...
    { name: 'Forex', type: 'FOREX' },
    { name: 'Indices', type: 'INDICES' },
    { name: 'Commodities', type: 'COMMODITIES' },
    { name: 'Synthetic', type: 'SYNTHETIC' },
    { name: 'BRVM', type: 'STOCKS' },
  ];
  ...
  const assets = [
    ...
    { symbol: 'SGBF',  name: 'Societe Generale BF', marketId: brvm.id, baseCurrency: 'XOF' },
  ...
  const emaTrendRsiRules = {
    ...
    rsi_bullish_zone: 45, rsi_bearish_zone: 55,
    min_confidence: 55, volume_spike_min: 1.3, atr_min_pct: 0.2,
    use_price_action: true, use_sr_zones: true, use_patterns: true,
    timeframes: ['1h', '4h'], trigger: 'BREAKOUT', profiles: ['SWING', 'DAY'],
  };
  ...
  await prisma.portfolio.upsert({
    ...
    update: {},
    create: { name: 'Mon Portfolio', type: 'PAPER', userId: admin.id },
  });
  console.log('Seed OK: markets, assets, strategy, super admin');
}
main().catch(e => { console.error('Seed failed:', e); process.exit(1); }).finally(() => prisma.\$disconnect());
" 2>&1 || echo "[entrypoint] Seed failed (may already exist)"
